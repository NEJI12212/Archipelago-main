...
CONNECTION_TIMING_OUT_STATUS = "Connection timing out. Please restart your emulator, then restart connector_gex_bizhawk.lua"
CONNECTION_REFUSED_STATUS = "Connection refused. Please start your emulator and make sure connector_gex_bizhawk.lua is running"
CONNECTION_RESET_STATUS = "Connection was reset. Please restart your emulator, then restart connector_gex_bizhawk.lua"
...
loc_name_to_id = network_data_package["games"]["gex"]["location_name_to_id"]
itm_name_to_id = network_data_package["games"]["gex"]["item_name_to_id"]
script_version: int = 1
version: str = "V0.1"
patch_md5: str = "34bb78448e2ed9c4236fad0f79f64448"
gvr_options = settings.get_settings().gex_options
program = None
...
async def patch_and_run(show_path):
  ...
  if not patch_path or existing_md5 != patch_md5:
    ...
    patch_successful = patch_rom(rom, patch_path, "assets/Gex.patch")
    if patch_successful:
  ...
  if patch_successful:
    ...
    if program_path and os.path.isfile(program_path) and (not program or program.poll() != None):
      ...
      if program_args:
        if program_args == "--lua=":
          lua = Utils.local_path("data", "lua", "connector_gex_bizhawk.lua")
          program_args = f'--lua={lua}'
          if os.access(os.path.split(lua)[0], os.W_OK):
            with open(lua, "w") as to:
              with open_world_file("assets/connector_gex_bizhawk.lua") as f:
...
class GexCommandProcessor(ClientCommandProcessor):
    def __init__(self, ctx):
        super().__init__(ctx)
    ...
    def _cmd_n64(self):
        ...
        if isinstance(self.ctx, GexContext):
...
class GexContext(CommonContext):
    command_processor = GexCommandProcessor
    ...
    async def server_auth(self, password_requested: bool = False):
        if password_requested and not self.password:
            await super(GexContext, self).server_auth(password_requested)
        if not self.auth:
    ...
    def run_gui(self):
        ...
        class GexManager(GameManager):
            logging_pairs = [
                ("Client", "Archipelago")
            ]
            base_title = "Gex Client "+ version + " for AP"
    ...
    def on_package(self, cmd, args):
        if cmd == "Connected":
            self.tracker = GexItemTracker(self)
            self.slot_data = args.get("slot_data", None)
            if version != self.slot_data["version"]:
                logger.error("Your Gex AP does not match with the generated world.")
                logger.error("Your version: "+version+" | Generated version: "+self.slot_data["version"])
                # self.event_invalid_game()
                raise Exception("Your Gex AP does not match with the generated world.\n" +
            ...
            if self.startup == False:
                ...
                logger.info("The above items will be sent when Gex is loaded.")
                self.startup = True
...
def get_payload(ctx: GexContext):
...
def get_slot_payload(ctx: GexContext):
    payload = json.dumps({
...
async def parse_payload(payload: dict, ctx: GexContext, force: bool):
    ...
    if ctx.deathlink_enabled:
        if payload["isDead"]: #gex died
            ctx.deathlink_pending = False
            if not ctx.deathlink_sent_this_death:
                ctx.deathlink_sent_this_death = True

                await ctx.send_death()
        else: # gex is somehow still alive
            ctx.deathlink_sent_this_death = False

    if ctx.taglink_enabled:
        if payload["isTag"]: #gex tagged
            ctx.pending_tag_link = False
...
async def n64_sync_task(ctx: GexContext):
...
def main():
    Utils.init_logging("Gex Client")
    parser = get_base_parser()
    args = sys.argv[1:]  # the default for parse_args()
    if "Gex Client" in args:
        args.remove("Gex Client")
    args = parser.parse_args(args)

    async def _main():
        multiprocessing.freeze_support()

        ctx = GexContext(args.connect, args.password)
